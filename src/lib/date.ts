...
export const getMondayIndexFromDate = (date: Date): number => {
  const jsDay = startOfDay(date).getDay()
  return (jsDay + 6) % 7
}

export const getLocalizedWeekdaysMondayFirst = (locale = 'en-US'): string[] => {
...
export const getAnchorWeekdayDate = (
  anchor: 'last' | 'next',
  weekdayIndexMonday: number,
  now = new Date(),
): Date => {
  const today = startOfDay(now)
  const todayMondayIndex = getMondayIndexFromDate(today)
...
export const getIsoWeekInfo = (date: Date): { weekNo: number; weekYear: number } => {
...
export const getDateMouseoverLabel = (dateValue: string, locale = 'en-US'): string => {
  ...
  const { weekNo, weekYear } = getIsoWeekInfo(parsed)
